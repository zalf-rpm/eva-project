...
# Letzte Änderung: 2011-09-23 12:00 (Xenia)
...
output_directory = "/home/specka/devel/git/models/monica/python/eva2/temperature_sum/"
#output_directory="C:/TempSum/"
#output_directory = "D:/Eigene Dateien/Temperatursumme/Ergebnisse/"

#######################################################
# Konfigurationsbereich
frucht="180"            # Id der Fruchtart angeben, für die die Temperatursummen berechnet werden sollen
crop_name = "futterhirse"
...
filename_missing_sowingdates = "missing_sowing_dates_" + crop_name + ".txt"

# --------------------------------------------------------------
       
"""
Sucht die Grenzen der Entwicklungsstadien
fuer die aktuelle Frucht raus
"""
def getDefStages(frucht):
    
    # TODO sollte aus der MONICA-Datenbank entnommen werden
    stages = []
    
    # min bbch, max_bbch, delta_bbch, base_temperature
    # delta_bbch gibt an, wie gross der Schwankungsbereich der BBCH
    # pro Stadium sein darf; je grÃƒÂ¶sser, desto ungenauer wird das ergebnis,
    # aber desto wahrscheinlich wird ein Datum für die BBCH-Bereich gefunden
    
    if (frucht== "160"): 
        # sudan gras, sorghum bicolor x sorghum sudanense      
        # min bbch, max_bbch, delta_bbch, base_temperature 
        stages.append([ 0, 9, 4, 12])
        stages.append([10,29, 3, 12])
        stages.append([30,49, 2, 10])
        stages.append([50,59, 3, 10])
        stages.append([60,69, 3, 10])
        stages.append([70,79, 3, 10])
        stages.append([80,99, 3, 10])

    elif (frucht == "180"):
        # Futterhirse, sorghum bicolor x sorghum bicolor      
        # min bbch, max_bbch, delta_bbch, base_temperature 
        stages.append([ 0, 9, 4, 12])
        stages.append([10,29, 3, 12])
        stages.append([30,49, 2, 10])
        ...
        # mais        
        # min bbch, max_bbch, delta_bbch, base_temperature
        stages.append([ 0, 9, 4, 7])
        stages.append([10,19, 3, 6])
        stages.append([31,39, 2, 6])
        stages.append([51,59, 3, 6])
        stages.append([60,69, 3, 9])
        stages.append([70,79, 3, 9])
        stages.append([80,99, 3, 9])    

    
    elif (frucht == "145"):      
        # Todo: Matthias bitte prÃ¼fen!  
        # spring barley        
        # min bbch, max_bbch, delta_bbch, base_temperature
        stages.append([ 0, 9, 4, 1])
        stages.append([10,25, 3, 1])
        stages.append([26,59, 2, 1])
        stages.append([60,69, 3, 1])
        stages.append([70,79, 3, 9])
        stages.append([80,99, 3, 9])
        ...
        # min bbch, max_bbch, delta_bbch, base_temperature
        stages.append([ 0, 9, 4, 1])
        stages.append([10,16, 3, 1])
        stages.append([17,60, 2, 1])
        stages.append([61,69, 3, 1])
        stages.append([70,89, 3, 9])
        stages.append([90,99, 3, 9])

    elif (frucht == "120" or frucht == "124"):        
        # oat or oat compound      
        # min bbch, max_bbch, delta_bbch, base_temperature
        stages.append([ 0, 9, 4, 1])
        stages.append([10,25, 3, 1])
...
def main():
    ...
    csv_writer = csv.writer(open(output_directory + "crop_" + str(frucht) + "_" + crop_name + ".txt", "wb"),delimiter=separator)
    csv__missing_writer = csv.writer(open(output_directory + filename_missing_sowingdates, "wb"),delimiter=separator)
