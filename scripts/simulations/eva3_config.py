...
def get_config(standort, fruchtfolge, anlage, klassifikation = 1):
    ...
    for row in rows:
        ...
        if (frucht != old_fruchtart or frucht == 141 or frucht == 176 or (frucht==180 and standort==44) or (frucht==172 and standort==19)):           
...
def get_ascha_config_gaerrestversuch(fruchtfolge, anlage, config):
    ...
    if (config.getVariante() == 2  or anlage == 3):
        config.setProfil_number(22) # 21-27 # was 23 before, but changed to 22 since better fit to measured values (AKP 18/02/2014)
...
def get_ascha_config_grundversuch(fruchtfolge, anlage, config):    
    if (config.getVariante() == 1  or anlage == 3):
        config.setProfil_number(25) # 21-27 # was 27 before, but changed to 25 since better fit to measured values (AKP 19/02/2014)
    elif (config.getVariante() == 2 or anlage == 4):        
        config.setProfil_number(22) # 21-27 # was 21 before, but changed to 22 since better fit to measured values (AKP 30/06/2014)   
...
def get_werlte_config_grundversuch(fruchtfolge, anlage, config):
    ...
    if (config.getVariante() == 1):
        ...
        config.setProfil_number(163) # AKP: nach Absprache mit F. Wilken (20.12.2013)
        
    elif (config.getVariante() == 4):
        config.setProfil_number(166) # AKP: nach Absprache mit F. Wilken (20.12.2013)
